#!/usr/bin/env python3
import json

rho0 = 1770.0

params = {
    "run_time_info": "T",
    "x_domain%beg": 0.0,
    "x_domain%end": 1.0,
    "y_domain%beg": 0.0,
    "y_domain%end": 0.02,
    "z_domain%beg": 0.0,
    "z_domain%end": 0.02,
    "m": 399,
    "n": 9,
    "p": 9,
    "dt": 1.0e-8,
    "t_step_start": 0,
    "t_step_stop": 600,
    "t_step_save": 600,
    "num_patches": 2,
    "model_eqns": 2,
    "num_fluids": 1,
    "mpp_lim": "F",
    "mixture_err": "F",
    "time_stepper": 3,
    "recon_type": 2,
    "muscl_order": 2,
    "muscl_lim": 2,
    "riemann_solver": 2,
    "wave_speeds": 1,
    "avg_state": 2,
    "bc_x%beg": -3,
    "bc_x%end": -3,
    "bc_y%beg": -3,
    "bc_y%end": -3,
    "bc_z%beg": -3,
    "bc_z%end": -3,
    "format": 1,
    "precision": 2,
    "prim_vars_wrt": "T",
    "parallel_io": "F",
    "patch_icpp(1)%geometry": 9,
    "patch_icpp(1)%x_centroid": 0.25,
    "patch_icpp(1)%y_centroid": 0.01,
    "patch_icpp(1)%z_centroid": 0.01,
    "patch_icpp(1)%length_x": 0.5,
    "patch_icpp(1)%length_y": 0.02,
    "patch_icpp(1)%length_z": 0.02,
    "patch_icpp(1)%vel(1)": 0.0,
    "patch_icpp(1)%vel(2)": 0.0,
    "patch_icpp(1)%vel(3)": 0.0,
    "patch_icpp(1)%pres": 30.0e9,
    "patch_icpp(1)%alpha_rho(1)": rho0,
    "patch_icpp(1)%alpha(1)": 1.0,
    "patch_icpp(2)%geometry": 9,
    "patch_icpp(2)%x_centroid": 0.75,
    "patch_icpp(2)%y_centroid": 0.01,
    "patch_icpp(2)%z_centroid": 0.01,
    "patch_icpp(2)%length_x": 0.5,
    "patch_icpp(2)%length_y": 0.02,
    "patch_icpp(2)%length_z": 0.02,
    "patch_icpp(2)%vel(1)": 0.0,
    "patch_icpp(2)%vel(2)": 0.0,
    "patch_icpp(2)%vel(3)": 0.0,
    "patch_icpp(2)%pres": 10.0e9,
    "patch_icpp(2)%alpha_rho(1)": rho0,
    "patch_icpp(2)%alpha(1)": 1.0,
    "fluid_pp(1)%eos": 2,
    "fluid_pp(1)%gamma": 2.5,
    "fluid_pp(1)%pi_inf": 0.0,
    "fluid_pp(1)%cv": 613.5,
    "fluid_pp(1)%jwl_A": 3.712e11,
    "fluid_pp(1)%jwl_B": 3.231e9,
    "fluid_pp(1)%jwl_R1": 4.15,
    "fluid_pp(1)%jwl_R2": 0.95,
    "fluid_pp(1)%jwl_omega": 0.30,
    "fluid_pp(1)%jwl_rho0": rho0,
    "fluid_pp(1)%jwl_E0": 1.0089e10,
    "fluid_pp(1)%jwl_air_e0": 2.5575e5,
    "fluid_pp(1)%jwl_air_rho0": 1.225,
    "fluid_pp(1)%jwl_air_gamma": 0.4,
}

print(json.dumps(params))
