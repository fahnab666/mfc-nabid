...
- `fluid_pp(i)%%eos` selects the equation of state of the $i$-th fluid. `stiffened_gas` (the default), `ideal_gas_mixture` (requires a chemistry build, backed by Pyrometheus), and `jwl` are currently supported; the enumeration reserves `mie_gruneisen` and `table` for future backends. A `jwl` fluid is the Jones-Wilkins-Lee principal isentrope used as a Mie-Gruneisen reference curve: it requires `model_eqns` = 2 and the six user-supplied coefficients `fluid_pp(i)%%jwl_A`, `jwl_B` (Pa), `jwl_R1`, `jwl_R2` (with $R_1 > R_2 > 0$), `jwl_omega` (the Gruneisen coefficient), and `jwl_rho0` (the reference charge density). Mixed JWL and stiffened-gas cells close with the exact pressure-equilibrium rule (see `mixture_closure`). No material catalog ships with MFC; coefficients are case inputs. A case file may load them from an external, provenance-tagged material file through `mfc.materials.material_fluid(i, spec, case_dir)`: a YAML file with a material name, an EOS family and its parameters, and provenance (a citation and a release status), searched (like Cantera mechanism files) as an explicit path, in the case directory, and in `$MFC_MATERIALS_DIR`. Files whose release status is not "synthetic", "open_literature", or "public" are refused. `examples/1D_jwl_products_expansion` is driven by a synthetic material file this way.
