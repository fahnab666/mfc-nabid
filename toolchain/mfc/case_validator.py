    ...
    def check_ibm(self):
        ...
        for i in range(1, num_particle_clouds + 1):
            ...
            if geometry == 2 and shell_outer_radius is not None and self._is_numeric(shell_outer_radius):
                axes = {
                    1: ("x", self.get(f"particle_cloud({i})%x_centroid", None), self.get("x_domain%beg", None), self.get("x_domain%end", None)),
                    2: ("y", self.get(f"particle_cloud({i})%y_centroid", None), self.get("y_domain%beg", None), self.get("y_domain%end", None)),
                    3: ("z", self.get(f"particle_cloud({i})%z_centroid", None), self.get("z_domain%beg", None), self.get("z_domain%end", None)),
                }
                # In 2D the shell degenerates to a half-annulus; the Fortran side falls back to opening
                # toward +y for any shell_axis other than x, so mirror that here.
                open_axis = shell_axis if (p > 0 or shell_axis == 1) else 2

                for axis in [1, 2, 3] if p > 0 else [1, 2]:
                    name, centroid, beg, end = axes[axis]
                    if not all(self._is_numeric(v) for v in [centroid, beg, end]):
                        continue
                    if axis == open_axis:
                        if not self._is_numeric(radius):
                            continue
                        # The shell opens toward +axis from its flat face at centroid; require one
                        # particle radius of standoff there and full outer-radius clearance on the open side.
                        self.prohibit(
                            centroid - radius < beg or centroid + shell_outer_radius > end,
                            f"particle_cloud({i}) hemisphere shell must clear {name}_domain by one particle radius on its flat face",
                        )
                    else:
                        self.prohibit(
