...
def list_cases() -> typing.List[TestCaseBuilder]:
        ...
        for slip in [True, False]:
            ...
            if len(dimInfo[0]) == 3:
                ...
                if slip and six_eqn_model:
                    ...
                    if cases[-1].mods.get("num_fluids", 1) == 2:
                        cases.append(
                            define_case_d(
                                stack,
                                "Moving Circle -> model_eqns=3 -> eos=mie_gruneisen",
                                {
                                    **cases[-1].mods,
                                    "dt": 0.002,
                                    "fluid_pp(1)%Re(1)": 1000.0,
                                    "fluid_pp(1)%Re(2)": 1000.0,
                                    "fluid_pp(2)%Re(1)": 1000.0,
                                    "fluid_pp(2)%Re(2)": 1000.0,
                                    "patch_ib(1)%moving_ibm": 1,
                                    "patch_ib(1)%vel(1)": 0.4,
    ...
    def foreach_example():
        for path in os.listdir(common.MFC_EXAMPLE_DIRPATH):
            ...
            casesToSkip = [
                ...
                # Needs external input/lag_bubbles.dat (same reason as the bubblescreens).
                "2D_jwl_progburn_el_bubbles",
                "2D_triple_point",
    ...
    def kernel_golden_tests():
        ...
        # The composition-weighted closure is applied unconditionally for JWL fluids.
        eps_jwl = 1e-8
        rho_jwl = 1630.0
        rho_air = 1.225
        ...
        # A homogeneous mixed slab exercises the weighted mixture coefficients at intermediate Y;
        ...
        # the pressure-equilibrium closure, not this single-fluid weight).
        rho_liq = 1000.0
        liq_gamma_mfc = 2.5
