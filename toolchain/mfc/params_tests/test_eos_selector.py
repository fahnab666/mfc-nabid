...
def test_existing_jwl_selector_keeps_the_equilibrium_parameters():
    assert EOS["jwl_pt"] == 2
    assert EOS["jwl"] != 2 and EOS["ideal_gas"] != 2
    assert _errors(**{"fluid_pp(1)%eos": 2, "fluid_pp(1)%jwl_A": 3.712e11, "fluid_pp(1)%jwl_E0": 7.0e9}) == []
    errors = _errors(**{"fluid_pp(1)%eos": EOS["jwl"], "fluid_pp(1)%jwl_E0": 7.0e9})
    assert any("require eos = 'jwl_pt'" in error for error in errors)
