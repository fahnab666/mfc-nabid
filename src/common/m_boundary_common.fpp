...
contains
    ...
    impure subroutine s_initialize_boundary_common_module(use_dirichlet_buffers)
        ...
        !> Populate the variable buffers along one direction and location, via MPI exchange for processor boundaries or by
        !! dispatching the per-cell BC routines over the boundary face.
        ...
        !> Populate ghost cell buffers for one IGR Jacobian BC direction and location, via MPI exchange for processor boundaries or
        !! by dispatching the per-cell IGR Jacobian BC routines over the boundary face.
        ...
        subroutine s_populate_grid_variables_buffers(x_cb_in, x_cc_in, dx_in, x_offset, y_offset, z_offset, y_cb_in, y_cc_in, &
            & dy_in, z_cb_in, z_cc_in, dz_in, global_bounds)
        ...
        subroutine s_populate_grid_bc_direction(cell_boundaries, cell_centers, cell_widths, num_cells, bc_dir, bc_loc, bc_bounds, &
            ...
            if (bc_edge >= 0) then
                call s_mpi_sendrecv_grid_variable_buffer(cell_boundaries, cell_centers, cell_widths, num_cells, bc_bounds, &
                    & bc_loc, offset)
                return
            end if
        ...
        !> Populate beta variable buffers for one direction and location, by dispatching the per-cell beta BC routines over the
        !! boundary face and performing the paired MPI reduction for processor boundaries.
