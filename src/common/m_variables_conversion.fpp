...
module m_variables_conversion
    ...
    use m_constants, only: riemann_solver_hll, riemann_solver_hlld, model_eqns_gamma_law, model_eqns_5eq, model_eqns_6eq, &
        & avg_state_roe, eos_stiffened_gas, eos_jwl
    ...
    public :: s_initialize_variables_conversion_module, s_initialize_pb, s_initialize_mv, s_convert_to_mixture_variables, &
        ...
        & f_pressure, s_phase_internal_energy, f_isentrope_exponent, f_isentrope_pressure, f_sg_thermal, &
    ...
    public :: s_compute_jwl_speed_of_sound
#endif
...
contains
    ...
    subroutine s_compute_pressure(energy, alf, dyn_p, pi_inf, gamma, rho, qv, rhoYks, pres, T, E_e_in, pres_mag, jwl_Y, jwl_lambda)
        ...
        real(wp), intent(in), optional :: jwl_Y, jwl_lambda
    ...
    subroutine s_convert_conservative_to_primitive_variables(qK_cons_vf, q_T_sf, qK_prim_vf, ibounds)
        ...
        $:GPU_PARALLEL_LOOP(collapse=3, private='[alpha_K, alpha_rho_K, Re_K, nRtmp, rho_K, gamma_K, pi_inf_K, qv_K, dyn_pres_K, &
                            & rhoYks, B, pres, Y_jwl, lambda_jwl, vftmp, nbub_sc, G_K, G_damaged, T, pres_mag, Ga, B2, m2, S, W, &
                            & dW, E, D, f, dGa_dW, dp_dW, df_dW, iter]')
                ...
                do j = ibounds(1)%beg, ibounds(1)%end
                    ...
                    #:if not MFC_CASE_OPTIMIZATION or jwl_active
                        if (jwl_idx > 0 .and. jwl_idx <= eqn_idx%cont%end) then
                            ...
                            lambda_jwl = 1._wp
                            if (jwl_reactive) lambda_jwl = min(max(qK_cons_vf(eqn_idx%rxn)%sf(j, k, l), 0._wp), 1._wp)
                            call s_compute_pressure(qK_cons_vf(eqn_idx%E)%sf(j, k, l), qK_cons_vf(eqn_idx%alf)%sf(j, k, l), &
                                                    ...
                                                    & pres_mag=pres_mag, jwl_Y=Y_jwl, jwl_lambda=lambda_jwl)
                        else
                        #:endif
                        call s_compute_pressure(qK_cons_vf(eqn_idx%E)%sf(j, k, l), qK_cons_vf(eqn_idx%alf)%sf(j, k, l), &
    ...
    !! @param[in] lambda jwl_reactive reaction progress; defaults to 1 (fully reacted).
