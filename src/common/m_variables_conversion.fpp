...
module m_variables_conversion
    ...
    use m_helper_basic
    use m_helper
    use m_constants, only: eos_stiffened_gas, eos_jwl
...
contains
    ...
    subroutine s_compute_pressure(energy, alf, dyn_p, pi_inf, gamma, rho, qv, rhoYks, pres, T, stress, mom, G, pres_mag, jwl_Y, &
        ...
        real(wp), intent(in), optional  :: jwl_lambda  !< JWL reaction progress; defaults to 1 (fully reacted).
        ...
        #:if not chemistry
            ...
            if ((jwl_idx > 0) .and. (.not. mhd) .and. (model_eqns /= 4) .and. (bubbles_euler .neqv. .true.)) then
                ! JWL five-equation mixture pressure via the Rocflu closure p(rho, e, Y, lambda).
                Y_jwl = 1._wp
    ...
    subroutine s_convert_conservative_to_primitive_variables(qK_cons_vf, q_T_sf, qK_prim_vf, ibounds)
        ...
        $:GPU_PARALLEL_LOOP(collapse=3, private='[alpha_K, alpha_rho_K, Re_K, nRtmp, rho_K, gamma_K, pi_inf_K, qv_K, dyn_pres_K, &
                            & rhoYks, B, pres, Y_jwl, lambda_jwl, vftmp, nbub_sc, G_K, T, pres_mag, Ga, B2, m2, S, W, dW, E, D, &
                            & f, dGa_dW, dp_dW, df_dW, iter ]')
                ...
                do j = ibounds(1)%beg, ibounds(1)%end
                    ...
                    if (jwl_idx > 0 .and. jwl_idx <= eqn_idx%cont%end) then
                        ...
                        call s_compute_pressure(qK_cons_vf(eqn_idx%E)%sf(j, k, l), qK_cons_vf(eqn_idx%alf)%sf(j, k, l), &
                                                ...
                                                & jwl_Y=Y_jwl, jwl_lambda=lambda_jwl)
                    else
                        call s_compute_pressure(qK_cons_vf(eqn_idx%E)%sf(j, k, l), qK_cons_vf(eqn_idx%alf)%sf(j, k, l), &
                                                & dyn_pres_K, pi_inf_K, gamma_K, rho_K, qv_K, rhoYks, pres, T, pres_mag=pres_mag)
                    end if
                    ...
                    ! JWL reaction-progress variables are advected material scalars (prim = cons).
            ...
            do k = 0, n
                do j = 0, m
                    ...
                    if (chemistry) then
                        ...
                        if (mhd) then
                        ...
                        else if (jwl_idx > 0 .and. (model_eqns /= 4) .and. (bubbles_euler .neqv. .true.)) then
                            ...
                            block
                                ...
                                lambda_j = 1._wp
                                call s_jwl_mix_energy_pr(rho, q_prim_vf(eqn_idx%E)%sf(j, k, l), Y_j, jwl_idx, e_mix_jwl, lambda_j)
                    ...
                    ! JWL reaction-progress variables are advected material scalars (cons = prim).
    ...
    subroutine s_convert_primitive_to_flux_variables(qK_prim_vf, FK_vf, FK_src_vf, is1, is2, is3, s2b, s3b)
        ...
        $:GPU_PARALLEL_LOOP(collapse=3, private='[alpha_rho_K, vel_K, alpha_K, Re_K, Y_K, rho_K, vel_K_sum, pres_K, E_K, gamma_K, &
                            & pi_inf_K, qv_K, G_K, T_K, mix_mol_weight, R_gas, e_mix_jwl, Y_j, lambda_j]')
        do l = is3b, is3e
            do k = is2b, is2e
                do j = is1b, is1e
                    $:GPU_LOOP(parallelism='[seq]')
                    ...
                    if (chemistry) then
                        ...
                        lambda_j = 1._wp
                        call s_jwl_mix_energy_pr(rho_K, pres_K, Y_j, jwl_idx, e_mix_jwl, lambda_j)
...
#ifndef MFC_PRE_PROCESS
    !> @brief Computes the JWL mixture speed of sound from explicit JWL fractions.
    ...
    !! @param[in] lambda JWL reaction progress; defaults to 1 (fully reacted).
    subroutine s_compute_jwl_speed_of_sound(pres, rho, Y_jwl, c, lambda)
        ...
        real(wp)                       :: Y_j, lambda_l

        Y_j = min(max(Y_jwl, 0._wp), 1._wp)
        lambda_l = 1._wp; if (present(lambda)) lambda_l = lambda

        call s_jwl_mix_sound_speed(rho, pres, Y_j, jwl_idx, c, lambda_l)

    end subroutine s_compute_jwl_speed_of_sound

    !> @brief Computes the speed of sound from thermodynamic state variables for non-JWL equation-of-state models.
