    ...
    impure subroutine s_check_jwl_inputs
        ...
        do i = 1, num_fluids
            ...
            if (fluid_pp(i)%eos == eos_jwl) then
                ...
                @:PROHIBIT(.not. f_is_default(fluid_pp(i)%jwl_ej_rho_ref) .and. fluid_pp(i)%jwl_ej_rho_ref <= 0._wp, &
                           & "fluid_pp(" // trim(iStr) // ")%jwl_ej_rho_ref must be positive when set")
                @:PROHIBIT(fluid_pp(i)%jwl_delta_e > 0._wp, &
                           & "fluid_pp(" // trim(iStr) &
                           & // ")%jwl_delta_e must be <= 0 (0 disables the reactant/product energy offset)")
                @:PROHIBIT(fluid_pp(i)%jwl_delta_e /= 0._wp .and. .not. jwl_reactive, &
                           & "fluid_pp(" // trim(iStr) &
                           & // ")%jwl_delta_e requires jwl_reactive (no reaction progress exists otherwise, so the offset would be silently ignored)")
            else
