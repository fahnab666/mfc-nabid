    ...
    subroutine s_compute_jwl_sources(q_cons_vf, rhs_vf)
        ...
        if (prog_burn) then
            ...
            if (r_front > 0._wp) then
                $:GPU_PARALLEL_LOOP(collapse=3, private='[r_det]')
                do l = 0, p
                    do k = 0, n
                        do j = 0, m
                            ...
                            r_det = sqrt(r_det)
                            if (r_det < r_front .and. r_det >= r_front - pb_width) then
        ...
        if (jwl_afterburn) then
            $:GPU_PARALLEL_LOOP(collapse=3, private='[i, rho, dyn_p, e_sp, Y, b, dbdt, pres, T, c]')
            do l = 0, p
                do k = 0, n
                    do j = 0, m
                        rho = 0._wp
                        ...
                        if (Y*(1._wp - Y)*(1._wp - b) > sgm_eps) then
                            if (jwl_ab_model == 1) then
                                ...
                                call s_jwl_mix_state_er(rho, e_sp, Y, jwl_idx, pres, T, c)
        ...
        if (jwl_reactive) then
            q_det = jwl_E0s(jwl_idx)/jwl_rho0s(jwl_idx)
            $:GPU_PARALLEL_LOOP(collapse=3, private='[i, rho, dyn_p, e_sp, Y, lambda, dldt, pres, T, c]')
            do l = 0, p
                do k = 0, n
                    do j = 0, m
                        rho = 0._wp
                        ...
                        if (Y*(1._wp - lambda) > sgm_eps) then
                            dyn_p = 0._wp
                            $:GPU_LOOP(parallelism='[seq]')
                            do i = eqn_idx%mom%beg, eqn_idx%mom%end
                                dyn_p = dyn_p + 0.5_wp*q_cons_vf(i)%sf(j, k, l)**2._wp/rho
