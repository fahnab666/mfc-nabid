...
contains
    ...
    subroutine s_hll_riemann_solver(qL_prim_rsx_vf, dqL_prim_dx_vf, dqL_prim_dy_vf, &
        ...
        real(wp)                  :: E_L, E_R
        real(wp)                  :: e_jwl_L, e_jwl_R, Y_jwl_L, Y_jwl_R, lambda_jwl_L, lambda_jwl_R
        real(wp)                  :: H_L, H_R
        ...
        #:for NORM_DIR, XYZ, STENCIL_VAR, COORDS, X_BND, Y_BND, Z_BND in &
            ...
            if (norm_dir == ${NORM_DIR}$) then
                $:GPU_PARALLEL_LOOP(collapse=3, private='[i, j, k, l, q, alpha_rho_L, alpha_rho_R, vel_L, vel_R, alpha_L, &
                                    ...
                                    & Y_jwl_R, lambda_jwl_L, lambda_jwl_R]', copyin='[norm_dir]')
                    ...
                    do k = ${Y_BND}$%beg, ${Y_BND}$%end
                        do j = ${X_BND}$%beg, ${X_BND}$%end
                            ...
                            if (chemistry) then
                                ...
                                ! JWL reactive burn requires HLLC, so this solver only sees fully-reacted states.
        ...
        #:for NORM_DIR, XYZ, STENCIL_VAR, COORDS, X_BND, Y_BND, Z_BND in &
            ...
            if (norm_dir == ${NORM_DIR}$) then
                $:GPU_PARALLEL_LOOP(collapse=3, private='[i, j, k, l, q, alpha_rho_L, alpha_rho_R, vel_L, vel_R, alpha_L, &
                                    ...
                                    & Y_jwl_L, Y_jwl_R, lambda_jwl_L, lambda_jwl_R]')
                do l = ${Z_BND}$%beg, ${Z_BND}$%end
                    do k = ${Y_BND}$%beg, ${Y_BND}$%end
                        do j = ${X_BND}$%beg, ${X_BND}$%end
                            $:GPU_LOOP(parallelism='[seq]')
            ...
            if (norm_dir == ${NORM_DIR}$) then
                ...
                if (model_eqns == 3) then
                    ...
                    $:GPU_PARALLEL_LOOP(collapse=3, private='[Re_max, i, q, T_L, T_R, vel_L_rms, vel_R_rms, pres_L, pres_R, &
                                        ...
                                        & G_R, e_jwl_L, e_jwl_R, Y_jwl_L, Y_jwl_R, lambda_jwl_L, lambda_jwl_R]', copyin='[is1, &
                                        & is2, is3]')
                    do l = ${Z_BND}$%beg, ${Z_BND}$%end
                        do k = ${Y_BND}$%beg, ${Y_BND}$%end
                            do j = ${X_BND}$%beg, ${X_BND}$%end
