...
contains
    ...
    subroutine s_mpi_sendrecv_ib_buffers(ib_markers, mpi_dir, pbc_loc)
        ...
        buffer_counts = (/buff_size*(n + 1)*(p + 1), buff_size*(m + 2*buff_size + 1)*(p + 1), &
                    ...
                    do l = 0, p
                        do k = 0, buff_size - 1
                            do j = -buff_size, m + buff_size
                                r = ((j + buff_size) + (m + 2*buff_size + 1)*(k + buff_size*l))
                ...
                #:else
                    ...
                    do l = 0, buff_size - 1
                        do k = -buff_size, n + buff_size
                            do j = -buff_size, m + buff_size
                                r = ((j + buff_size) + (m + 2*buff_size + 1)*((k + buff_size) + (n + 2*buff_size + 1)*l))
            ...
            if (rdma_mpi .eqv. ${'.true.' if rdma_mpi else '.false.'}$) then
                #:if rdma_mpi
                    #:call GPU_HOST_DATA(use_device_addr='[ib_buff_send, ib_buff_recv]')
                        ...
                        call MPI_SENDRECV(ib_buff_send, buffer_count, MPI_INTEGER, dst_proc, send_tag, ib_buff_recv, &
                                          & buffer_count, MPI_INTEGER, src_proc, recv_tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE, ierr)
                        call nvtxEndRange
                ...
                #:else
                    ...
                    call MPI_SENDRECV(ib_buff_send, buffer_count, MPI_INTEGER, dst_proc, send_tag, ib_buff_recv, buffer_count, &
                                      & MPI_INTEGER, src_proc, recv_tag, MPI_COMM_WORLD, MPI_STATUS_IGNORE, ierr)
                    call nvtxEndRange
            ...
            if (mpi_dir == ${mpi_dir}$) then
                ...
                #:elif mpi_dir == 2
                    ...
                    do l = 0, p
                        do k = -buff_size, -1
                            do j = -buff_size, m + buff_size
                                r = ((j + buff_size) + (m + 2*buff_size + 1)*((k + buff_size) + buff_size*l))
    ...
    impure subroutine s_add_particles_to_transfer_list(nBub, pos, posPrev, include_ghost)
        ...
        do k = 1, nbub
                ...
                if (n > 0) then
                    if (f_crosses_boundary(k, 2, -1, pos, posPrev, include_ghost)) then
                        ...
                        if (p > 0) then
                            ...
                            else if (f_crosses_boundary(k, 3, 1, pos, posPrev, include_ghost)) then
                                call s_add_particle_to_direction(k, -1, -1, 1)
                                call s_add_particle_to_direction(k, 0, -1, 1)
                                call s_add_particle_to_direction(k, -1, 0, 1)
                                call s_add_particle_to_direction(k, 0, 0, 1)
                    ...
                    else if (f_crosses_boundary(k, 2, 1, pos, posPrev, include_ghost)) then
                        call s_add_particle_to_direction(k, -1, 1, 0)
                        call s_add_particle_to_direction(k, 0, 1, 0)
                        if (p > 0) then
                            if (f_crosses_boundary(k, 3, -1, pos, posPrev, include_ghost)) then
            ...
            else if (f_crosses_boundary(k, 1, 1, pos, posPrev, include_ghost)) then
                call s_add_particle_to_direction(k, 1, 0, 0)
                if (n > 0) then
                    if (f_crosses_boundary(k, 2, -1, pos, posPrev, include_ghost)) then
                        call s_add_particle_to_direction(k, 1, -1, 0)
    ...
    contains
        ...
        logical function f_crosses_boundary(particle_id, dir, loc, pos, posPrev, include_ghost)
            ...
            if (present(include_ghost) .and. include_ghost) then
