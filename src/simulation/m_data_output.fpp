...
contains
    ...
    impure subroutine s_write_run_time_information(q_prim_vf, t_step)
        ...
        $:GPU_PARALLEL_LOOP(collapse=3, private='[j, k, l, Y_jwl, lambda_jwl, vel, alpha, alpha_rho, Re, rho, vel_sum, pres, &
                            & gamma, pi_inf, c, qv, icfl, vcfl, Rc, ccfl, fl, mu_frac]', reduction='[[icfl_max_loc, vcfl_max_loc, &
    ...
    impure subroutine s_report_icfl_violation(q_prim_vf)
        ...
        real(wp)                                            :: rho, vel_sum, pres, gamma, pi_inf, qv, c, Y_jwl, lambda_jwl
        ...
        scan: do l = 0, p
            do k = 0, n
                do j = 0, m
                    ...
                    if (jwl_idx > 0) then
                        Y_jwl = min(max(alpha_rho(jwl_idx)/max(rho, sgm_eps), 0._wp), 1._wp)
                        lambda_jwl = 1._wp
                        if (jwl_reactive) lambda_jwl = min(max(q_prim_vf(eqn_idx%rxn)%sf(j, k, l), 0._wp), 1._wp)
                        call s_compute_jwl_speed_of_sound(pres, rho, Y_jwl, c, lambda_jwl)
                    else
