...
module m_checker
    ...
    use m_helper
    use m_helper_basic
    use m_constants, only: eos_jwl, model_eqns_5eq, BC_CHAR_SLIP_WALL, BC_CHAR_SUP_OUTFLOW
...
contains
    ...
    impure subroutine s_check_inputs
        ...
        if (any(fluid_pp(1:num_fluids)%eos == eos_jwl)) then
            @:PROHIBIT(wave_speeds == 2, "wave_speeds = 2 (pressure-based) is not supported with eos_jwl; use wave_speeds = 1")
        ...
        ! m_bubbles.fpp): the 5-equation model supplies no Tait constants, and the
        ! Euler-Lagrange path passes dummy ntait/Btait that are never assigned, so
        ! Gilmore is invalid there for any flow model.
        @:PROHIBIT(bubbles_euler .and. model_eqns == model_eqns_5eq .and. bubble_model == 1, &
                   & "The 5-equation bubbly flow model does not support bubble_model = 1 (Gilmore)")
        @:PROHIBIT(bubbles_lagrange .and. bubble_model == 1, &
