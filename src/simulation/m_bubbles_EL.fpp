...
contains
    ...
    impure subroutine s_initialize_bubbles_EL_module(q_cons_vf, bc_type)
        ...
        if (lag_params%vel_model > 0 .and. lag_params%pressure_force) then
            ...
            @:ALLOCATE(fd_coeff_x_pgrad(-fd_number:fd_number,-fd_number:m_alloc + fd_number))
            ...
            if (n > 0) then
                ...
                @:ALLOCATE(fd_coeff_y_pgrad(-fd_number:fd_number,-fd_number:n_alloc + fd_number))
            ...
            if (p > 0) then
                ...
                @:ALLOCATE(fd_coeff_z_pgrad(-fd_number:fd_number,-fd_number:p_alloc + fd_number))
    ...
    subroutine s_compute_cson_from_pinf(q_prim_vf, pinf, cell, rhol, gamma, pi_inf, cson)
        ...
        #:if not MFC_CASE_OPTIMIZATION or jwl_active
            if (jwl_idx > 0) then
                ...
                lambda_j = 1._wp
                if (jwl_reactive) lambda_j = min(max(q_prim_vf(eqn_idx%rxn)%sf(cell(1), cell(2), cell(3)), 0._wp), 1._wp)
                call s_jwl_mix_sound_speed(rhol, pinf, Y_j, jwl_idx, cson, lambda_j)
            else
