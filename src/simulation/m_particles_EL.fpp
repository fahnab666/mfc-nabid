...
contains
    ...
    impure subroutine s_initialize_particles_EL_module(q_cons_vf, bc_type)
        ...
        $:GPU_UPDATE(device='[cp_particle, rho0ref_particle]')
    ...
    subroutine s_compute_particle_EL_dynamics(q_cons_vf, q_prim_vf, bc_type, stage, rhs_vf)
        ...
        #:if not MFC_CASE_OPTIMIZATION and USING_AMD
            ...
            real(wp), dimension(3) :: alpha_rho  !< Cell-avg. phasic density
            real(wp), dimension(3) :: alpha_cell, alpha_rho_cell
        #:else
            real(wp), dimension(num_vels)   :: vel        !< Cell-avg. velocity
            real(wp), dimension(num_fluids) :: alpha      !< Cell-avg. volume fraction
            real(wp), dimension(num_fluids) :: alpha_rho  !< Cell-avg. phasic density
            real(wp), dimension(num_fluids) :: alpha_cell, alpha_rho_cell
        #:endif
        ...
        if (lag_params%pressure_force .or. lag_params%added_mass_model > 0) then
            do l = 1, num_dims
                call s_gradient_dir_fornberg(q_prim_vf(eqn_idx%E)%sf, field_vars(dPx_id + l - 1)%sf, l)
                do dir = 1, num_dims
                    call s_gradient_dir_fornberg(q_prim_vf(eqn_idx%mom%beg + dir - 1)%sf, field_vars(duidxj_id(dir, l))%sf, l)
                end do
            end do
        end if
        ...
        if (lag_params%added_mass_model > 0) then
            do l = 1, num_dims
                call s_gradient_dir_fornberg(q_prim_vf(eqn_idx%cont%beg)%sf, field_vars(drhox_id + l - 1)%sf, l)
            end do
        end if
        ...
        do k = 1, n_el_particles_loc
            ...
            call s_interp_fluid_properties(myPos, cell, q_prim_vf, weights_x_interp, weights_y_interp, weights_z_interp, &
                                           & myFluidVel, myFluidRho, myPres, alpha, alpha_rho)
    ...
    subroutine s_get_drag_viscosity(q_prim_vf, fluid_pres, fluid_rho, pi_inf_mix, alpha, alpha_rho, Re_mix, mu, i, j, k)
        ...
        real(wp)                                            :: mu_f
        real(wp)                                            :: fluid_temp, mix_mol_weight, rho_phase
        integer                                             :: l, d
            ...
            if (viscous) then
                ...
                do l = 1, num_fluids
                    ...
                    if (lag_params%suth(l) > 0._wp) then
                        rho_phase = alpha_rho(l)/max(alpha(l), sgm_eps)
                        call s_phase_temperature(rho_phase, fluid_pres, l, fluid_temp)
